...
function initTriangle() {
	...
	mixed = [ -1, -1, 0, 1, 1, -1, 1, 0, 0, 1, 1, 1, 0, 1, 0, 0, 1, 1 ];
...
function initTriangleBuffers() {
	...
	// 5.1 Create VBO for positions and activate it
	poscolVBO = gl.createBuffer();
	gl.bindBuffer(gl.ARRAY_BUFFER, poscolVBO);
	...
	// 6.2 Fill VBO with colors
	//gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(mixed), gl.STATIC_DRAW);
}
...
function renderTriangle() {
	...
	gl.vertexAttribPointer(colorLoc, 4, gl.FLOAT, false, 0, 24);
