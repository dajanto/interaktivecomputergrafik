...
function meshConverter(model) {
    ...
    if (model.meshes[0].colors) {
        ...
        switch (n) {
            case 0:
                colors = model.meshes[0].vertices.map(x => [0.6, 0.3, 0, 1]).flat()
                break;
            case 1:
                colors = model.meshes[0].vertices.map(x => [0, 0, 0, 0.3]).flat()
                break;
            case 2:
                colors = model.meshes[0].vertices.map(x => [0, 0, 0, 0.2]).flat()
                break;
            case 3:
                colors = model.meshes[0].vertices.map(x => [0.1, 0.25, 0.05, 1]).flat()
                break;
            case 4:
                colors = model.meshes[0].vertices.map(x => [0.1, 0.25, 0.05, 1]).flat()
                break;
...
function move(e) {
    ...
    switch (e.key) {
        case "w" :
            glMatrix.vec3.negate(normalized, look);
            glMatrix.vec3.normalize(normalized, normalized);
            break;
        case "s" :
            glMatrix.vec3.normalize(normalized, look);
            break;
        case "a" :
            glMatrix.vec3.normalize(normalized, strafe);
            break;
        case "d" :
            glMatrix.vec3.negate(normalized, strafe);
            glMatrix.vec3.normalize(normalized, normalized);
            break;
        default :
            return;
